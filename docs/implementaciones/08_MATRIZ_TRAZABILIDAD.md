...
| **H-17** | **Validación de marcas de versión (jul. 2026):** los scripts web renovados registran `V. fecha.N + FechaUpdate` en JSON (`parametros_web.ScriptsWeb`); se añadió un paso de validación en los 3 paso a paso (C1 P7, C2 P3, C3 P6) para detectar **scripts olvidados**; en C2 además detecta NEW SAG desactualizada → escalar al Líder. Script 6 oficializado en `SQLs Web/6.` con marca distinguida ("SOLO"). Documento de error "cliente cambió correos" **rechazado** (no impide el acceso). | RESUELTO (implementado por el negocio; documentado) | Plantillas con `c*.b.validarScripts`; RN-20 nueva; [SQL] §F es la referencia del mecanismo. |
...
| Validación de marcas de scripts | Paso 7 (5 marcas, sin "SOLO") | Paso 3 (4 marcas; vacía=esperado; versión NEW SAG → escalar) | Paso 6 (4 marcas + "SOLO") | ✔ [SQL] §F |
