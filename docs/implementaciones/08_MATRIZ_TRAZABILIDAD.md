...
| RN-02 (activos ≤ contratados; cupo POR CLIENTE contando únicos por correo) | M2 §3 (`contractedUsers` obligatorio C1), M3 §2 pasos `c1.a.entregables`/`c1.a.barridoUsuarios`, M4 §3 (correo 2 C1) | ✔ |
...
| RN-11 v2 (extracción por ss_email DENTRO del barrido, antes de los scripts; sin reimportación en producción) | M3 `c1.a.barridoUsuarios` + `c1.f.prepararProduccion` | ✔ |
...
| **H-17** | **Validación de marcas de versión (jul. 2026):** los scripts web renovados registran `V. fecha.N + FechaUpdate` en JSON (`parametros_web.ScriptsWeb`); se añadió un paso de validación en los 3 paso a paso (C1 P5, C2 P3, C3 P5) para detectar **scripts olvidados**; en C2 además detecta NEW SAG desactualizada → escalar al Líder. Script 6 oficializado en `SQLs Web/6.` con marca distinguida ("SOLO"). Documento de error "cliente cambió correos" **rechazado** (no impide el acceso). | RESUELTO (implementado por el negocio; documentado) | Plantillas con `c*.b.validarScripts`; RN-20 nueva; [SQL] §F es la referencia del mecanismo. |
| **H-16** | **Simplificación implementada (jul. 2026): importación CSV de usuarios en C1.** La creación/asociación manual en SAG Admin fue reemplazada por una importación desde el Detalle del cliente (crea nuevos con contraseña autogenerada, omite existentes/inválidos, asocia todos, descarga Excel de resultados = insumo de credenciales). Query de extracción ahora con NúmeroTelefono; nuevos scripts 08/09/10 en la carpeta C1; nuevo paso STOP de formato de correos (FASE A) espejado al cliente como Consulta 3. | RESUELTO (ya implementado y probado por el negocio) | Plantilla C1 actualizada (`c1.a.correosInvalidos`, `c1.b.extraerUsuarios` CSV+conteo, `c1.c.importarUsuarios`); RN-03/RN-11 actualizadas. **Extendida a C3 el 2026-07-09** (plantilla + `GenerarCSVUsuarios.exe` + `c3.c.importarUsuarios` — ver H-18). |
| **H-18** | **Rediseño del 2026-07-09 (plan de 14 correcciones):** C1 quedó en **14 pasos** con el **BARRIDO único** (`c1.a.barridoUsuarios`: consulta ÚNICA de correos + extracción por ss_email + cupo de únicos con `ConsolidarUsuarios.exe` + reporte único; correcciones en PRODUCCIÓN → copia fresca → repetir); **sin reimportación en producción**; credenciales en **2 envíos** (RN-24); C3 renovado (hoy **15 pasos** tras fusionar la validación de correos en el Paso 2, 2026-07-16; permisos del admin CONF/USU/USU2 + plantilla→CSV con `GenerarCSVUsuarios.exe` + importación — RN-22); prerrequisitos C3 lineales con plantilla adjunta y nombre de BD en nube; solicitud a Elasticserver C3 con 2 variantes (producción SIEMPRE; pruebas salvo BI/portales — Excel vivo `Licencias sin ambiente de pruebas.xlsx`); Cliente Nuevo HÍBRIDO con RDPs (RN-23); Visios con inicio = LA SOLICITUD; entregables renumerados 00–09 (01 = enlace a Opciones Disponibles; checklist = 04; paso a paso = 06; credenciales = 07; guía = 08). | RESUELTO (documentos y herramientas actualizados) | RN-21..RN-24 nuevas; plantillas M3 actualizadas (c1/c3); el motor debe modelar el bucle del barrido y el flag híbrido. |
| **H-19** | **Decisiones del 2026-07-16 (reuniones William / William+Alejandro):** (a) credenciales en **DOS correos numerados** (4a-1 usuarios / 4a-2 contraseñas) + confirmación 4b, generados por la herramienta `GenerarCorreosCredenciales` (3 instancias) — supersede los ENVÍOs, las 3 plantillas de C3 y el esquema "sin contraseñas" aprobado en llamada; (b) **SIEMPRE hay pruebas**: los módulos de consulta (portales) prueban sobre la BD de producción — la rama de C3 es "¿copia de pruebas?" (RF-12 v2); (c) **BI sale de C3**: implementación propia (carpeta pendiente; el cliente entrega solo la lista de usuarios; P&A administra los usuarios); (d) **híbrido v2 APLICADO**: Ventas notifica, SOPORTE solicita/entrega los RDP (RN-23 v2); (e) **Fuerza de Ventas = módulo especial** (usa C3, sin carpeta propia); (f) la consulta de validación no puede decir "usuario" ni en los literales → "tercero(s)"; (g) reglas del check de opciones (Comité de Control de Cambios; dudas → correo a William, quien actualiza el archivo; check temporal). | RESUELTO (documentos, plantillas y herramienta actualizados) | RF-12/RN-23/RN-24 reescritas; correos 4a-1/4a-2/4b en `05`; el motor debe modelar la tarea de RDPs de Soporte y la futura plantilla de BI. |
...
| Validación de marcas de scripts | Paso 5 (5 marcas, sin "SOLO") | Paso 3 (4 marcas; vacía=esperado; versión NEW SAG → escalar) | Paso 6 (4 marcas + "SOLO") | ✔ [SQL] §F |
