...
- **Fila 7b (FASE C solo Líder):** [PROC] §1.B FASE C está marcada "· Líder de Operaciones *(← handoff desde Servicio al Cliente)*". El proceso separa explícitamente esa responsabilidad; la matriz lo refleja para que el software refuerce el handoff interno. **Matiz:** manda el `responsibleRole` de cada paso en la plantilla — los pasos de **SAG Admin** son del Líder, pero p. ej. "obtener la cadena de conexión" (C2 Paso 3) es de Servicio al Cliente según el docx definitivo.
- **Filas 8 y 10 (credenciales solo Líder):** CONFIRMADO por Juan Camilo (jul. 2026) y por el docx de C2 (Paso 11, responsable "Líder de Operaciones"): las credenciales al cliente **siempre** las envía el Líder de Soporte, en los tres casos.
