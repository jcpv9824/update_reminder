...
| `c1.a.barridoUsuarios` | A | **BARRIDO único (RN-21):** por cada BD de compañía: (a) la consulta ÚNICA de validación de correos (un solo SELECT — `validacion_correos.sql`; cada fila trae Problema/Detalle/Que_hacer) — se ANOTA, no se detiene; (b) extracción del CSV (`consulta_importacion_usuarios.sql` por **ss_email** — RN-11 v2); (c) `ConsolidarUsuarios.exe` → **únicos por correo vs. contratados POR CLIENTE**; (d) reporte único → verde = continuar / algo mal = **STOP**: cliente corrige en PRODUCCIÓN → copia fresca → repetir barrido | ✔ | support | [PROC] §1.B.2 (2026-07-09) |
...
| `c3.a.validarCorreos` | A | **Consulta ÚNICA de validación de correos** (duplicados, vacíos y formato en un solo SELECT — `validacion_correos.sql`) — **SÍ aplica** (hay usuarios previos en Clásico — RN-03). STOP | ✔ | support | [PROC] §3.B, [SQL] §D, docx C3 Paso 2 (2026-07-16) |
...
| `c3.b.validarScripts` | B | **Validar marcas de versión**: 4 marcas web con FechaUpdate de hoy + `migrar_usuarios` CON "SOLO migracion de login/emails" (la escribe el script 6; si está vacía, faltó correrlo y los usuarios no podrán iniciar sesión) | ✔ | support | docx C3 Paso 5 (jul. 2026), [SQL] §F |
| `c3.b.generarCsv` | B | **Generar el CSV Y los scripts desde la(s) plantilla(s)** con `GenerarCSVUsuarios.exe`: entrada sin ID real (Nombres/Apellidos/Correo/Admin); detecta encabezados, normaliza y valida; crea ID técnico secuencial. En verde deja CSV + scripts por compañía + reporte en `Resultado_GenerarCSVUsuarios`. Con UN error limpia CSV/scripts anteriores, deja solo el reporte con hoja `Problemas` y el paso no puede avanzar (RN-22 v2). | ✔ | support | docx C3 Paso 6 (2026-07-14), RN-22 |
| `c3.b.permisosAdmin` | B | **Ejecutar los scripts de permisos generados** (`asignar_permisos_admins_{COMPAÑÍA}.sql`) en la BD de cada compañía — UNA SOLA PASADA para todos los admins (CONF/USU/USU2, idempotente; requiere scripts 1–2). Los correos no encontrados se reportan sin detener el script → corregir en la plantilla y re-ejecutar (RN-22) | ✔ | support | docx C3 Paso 7 (2026-07-10) |
...
| `c3.c.importarUsuarios` | C | **Importar los usuarios por CSV** en SAG Admin (Detalle del cliente → importación → procesar; `resultado.xlsx` = insumo de credenciales). Supersede la creación manual (`c3.c.usuariosModulo`) | ✔ | **lead** | docx C3 Paso 11 (2026-07-09), RN-22 |
...
2. **Evidencia obligatoria en pasos STOP:** completar `c1.a.barridoUsuarios`/`c3.a.validarCorreos` exige evidencia (resultado de la consulta ÚNICA). *Justificación:* [SQL] §D — la mitad de los errores comunes de Soporte nacen de saltarse estas validaciones.
