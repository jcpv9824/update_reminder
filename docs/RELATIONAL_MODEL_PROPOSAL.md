...
6. **Los archivos salen de la fila operativa.** Los Base64 actuales de formatos y descargas se migran a almacenamiento privado seleccionable SeaweedFS mediante su gateway S3 o Azure Blob. SQL conserva proveedor, locator, metadata, hash y versión. El switch controla nuevas escrituras y los objetos existentes siguen leyéndose según su proveedor persistido. Esto evita inflar backups, log y memoria de Azure Functions.
...
`content.files`: PK `file_key`; public ID `UNIQUEIDENTIFIER`; `storage_provider`; locator Azure (`storage_container`, `blob_name`) o S3 (`storage_bucket`, `object_key`); `object_etag`, `original_name`, `mime_type`, `byte_length`, `sha256 BINARY(32)`, `created_at/by`. Cada carga es inmutable. No se guarda Base64, credencial ni URL firmada.
...
| Archivos | Cerrada: proveedor privado seleccionable SeaweedFS S3 o Azure Blob + metadata SQL; no `VARBINARY(MAX)`. El switch afecta nuevas escrituras, no reinterpreta locators históricos. |
