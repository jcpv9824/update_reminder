...
- Selectable private object storage: provider-managed SeaweedFS through its S3 gateway or Azure Blob through managed identity. The runtime switch selects new writes while SQL locators keep historical reads provider-aware.
...
- SQL Server plus selectable private SeaweedFS S3-gateway or Azure Blob storage for operational persistence; Key Vault for application secrets.
