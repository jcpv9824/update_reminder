...
Configure sesiones seguras sin imprimir secretos:
...
No regenere `JWT_SECRET` en cada redeploy. El comando anterior es para aprovisionamiento inicial o rotacion controlada. Consulte `SECURITY_SESSIONS.md`.
...
Para SEC-007 configure `BCRYPT_COST=12`, `PASSWORD_MAX_AGE_DAYS=180`, `PWNED_PASSWORDS_ENABLED=true` y `PWNED_PASSWORDS_FAIL_CLOSED=true`. El acceso usa correo y contraseña sin segundo factor. Las variables heredadas `MFA_ISSUER` y `MFA_RECOVERY_PEPPER` deben retirarse después de desplegar el backend actualizado. Consulte `SECURITY_PASSWORD_POLICY.md`.
